...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function setTheme(next){ document.documentElement.setAttribute("data-theme", next); try{localStorage.setItem(key, next);}catch(e){} if(btn){btn.setAttribute("aria-pressed", next==="dark"?"true":"false"); const label = btn.querySelector("[data-theme-label]"); if(label) label.textContent = next==="dark"?"Dark":"Light";} const iframe = document.querySelector("iframe.giscus-frame"); if(iframe){ iframe.contentWindow.postMessage({ giscus:{ setConfig:{ theme: (next==="dark"?"dark":"light") } } }, "https://giscus.app"); } }
  setTheme(getTheme());
